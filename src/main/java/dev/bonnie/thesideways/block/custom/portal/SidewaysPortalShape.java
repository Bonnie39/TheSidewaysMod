...
public class SidewaysPortalShape {
    private static final BlockBehaviour.StatePredicate FRAME = (state, level, pos) -> state.is(ModTags.Blocks.PORTAL_FRAME_BLOCKS);
    ...
    public static Optional<SidewaysPortalShape> findEmptySidewaysPortalShape(LevelAccessor level, BlockPos bottomLeft, Direction.Axis axis) {
        return findPortalShape(level, bottomLeft, (shape) -> shape.isValid() && shape.numPortalBlocks == 0, axis);
    }

    public static Optional<SidewaysPortalShape> findPortalShape(LevelAccessor level, BlockPos bottomLeft, Predicate<SidewaysPortalShape> predicate, Direction.Axis axis) {
        Optional<SidewaysPortalShape> optional = Optional.of(new SidewaysPortalShape(level, bottomLeft, axis)).filter(predicate);
        if (optional.isPresent()) {
            return optional;
        } else {
            Direction.Axis directionAxis = axis == Direction.Axis.X ? Direction.Axis.Z : Direction.Axis.X;
            return Optional.of(new SidewaysPortalShape(level, bottomLeft, directionAxis)).filter(predicate);
        }
    }

    public SidewaysPortalShape(LevelAccessor level, BlockPos bottomLeft, Direction.Axis axis) {
        this.level = level;
        this.axis = axis;
    ...
    private int getDistanceUntilTop(BlockPos.MutableBlockPos mutablePos) {
        for (int i = 0; i < 21; ++i) {
            ...
            for (int j = 0; j < this.width; ++j) {
                ...
                if (blockState.is(ModBlocks.SIDEWAYS_PORTAL.get())) {
    ...
    private static boolean isEmpty(BlockState state) {
        return state.isAir() || state.is(Blocks.WATER) || state.is(ModBlocks.SIDEWAYS_PORTAL.get());
    }
    ...
    public void createPortalBlocks() {
        BlockState blockState = ModBlocks.SIDEWAYS_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, this.axis);
