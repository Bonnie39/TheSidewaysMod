 ...
 */
public class SidewaysLakeFeature extends Feature<SidewaysLakeConfiguration> {
    private static final BlockState AIR = Blocks.CAVE_AIR.defaultBlockState();

    public SidewaysLakeFeature(Codec<SidewaysLakeConfiguration> codec) {
        super(codec);
    }

    public boolean place(FeaturePlaceContext<SidewaysLakeConfiguration> context) {
        BlockPos blockPos = context.origin();
        WorldGenLevel level = context.level();
        RandomSource random = context.random();
        SidewaysLakeConfiguration sidewaysLakeConfiguration = context.config();
        if (blockPos.getY() <= level.getMinBuildHeight() + 4) {
            ...
            BlockState fluidBlockState = sidewaysLakeConfiguration.fluid().getState(random, blockPos);
            ...
            BlockState topBlockState = sidewaysLakeConfiguration.top().getState(random, blockPos);
